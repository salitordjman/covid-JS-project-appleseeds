...
for(let objCode in objCountriesCode){ 
    start.style.visibility="visible"
    let coronaApi = await axios
    .get(`https://intense-mesa-62220.herokuapp.com/https://corona-api.com/countries/${objCode}`)
            start.style.visibility="hidden"
            const dataToObj=coronaApi.data.data
            Object.assign(objCountriesCode[objCode], {
                name: dataToObj.name,
                confirmed: dataToObj.latest_data.confirmed,
                critical: dataToObj.latest_data.critical,
                deaths: dataToObj.latest_data.deaths,
                recovered: dataToObj.latest_data.recovered,
                todayConfirmed: dataToObj.today.confirmed,
                todayDeaths: dataToObj.today.deaths
            });
//!Creating and preserving countries that exist on the same continent
            ...
            countriesInfoMenu.appendChild(specificInfo)
                specificInfo.innerHTML = dataToObj.name;
